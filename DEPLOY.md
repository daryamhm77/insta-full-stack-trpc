...
If you prefer GitOps, connect the repo as a [Blueprint](https://render.com/docs/infrastructure-as-code) using `render.yaml`. Set the `sync: false` variables in the dashboard (Vercel URL + Cloudinary keys) before the first deploy succeeds.
